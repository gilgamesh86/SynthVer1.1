...
flag scan = 0; // matrix scan flag
flag adsrTick = 0;
flag releaseFlag = 0;
flag ledFlag = 0;
flag gachaFlag = 0; // ignore this for now
...
#define DEBOUNCE_MS 150 // unusually high but lower values just dont work
...
int main(void)
{
  ...
  HAL_TIM_Base_Start_IT(&htim4); // matrix scan timer, 1000hz
  HAL_TIM_Base_Start_IT(&htim6); // adsr ticks, 10000hz
  HAL_TIM_Base_Start_IT(
      &htim7); // blinky timer (kasane teto mode, comming soon)
  HAL_TIM_Encoder_Start(&htim2, TIM_CHANNEL_ALL); // encoder mode timer
  displayInit();
  ...
  while (1) {
    ...
    if (updateState == 1) {
      redraw = updateState; // this + function set as bool refreshes screen only
